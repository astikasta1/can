...
import {
  ...
  ArrowRight, HardHat, ClipboardCheck,
...
function Section({ children, id, className = '' }) {
  ...
  return (
    <section
      id={id}
      ref={ref}
      className={`transition-all duration-700 motion-reduce:transform-none motion-reduce:transition-none ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} ${className}`}
    >
      {children}
    </section>
  );
...
function clampArea(value) {
  ...
  return Math.min(200, Math.max(20, Math.round(numeric)));
}
...
export default function App() {
  ...
  const toggle = (title) => setSelected(prev => ({ ...prev, [title]: !prev[title] }));
  const total = services.reduce((sum, service) => sum + (selected[service.title] ? service.price * area : 0), 0);

  /* reviews slider */
  const [slide, setSlide] = useState(0);
  const next = () => setSlide(prev => (prev + 1) % reviews.length);
  const prev = () => setSlide(prev => (prev - 1 + reviews.length) % reviews.length);
  ...
  const handleSubmit = (e) => {
    ...
    if (!isNameValid) {
      setFormError('Укажите имя, чтобы мы знали, как к вам обращаться.');
      return;
    }

    if (!isPhoneValid) {
      setFormError('Укажите телефон в формате +7 (___) ___-__-__.');
      return;
    }
    ...
    <div className="min-h-screen bg-anthracite-950 text-gray-200 font-sans overflow-x-hidden">
      <nav className="fixed top-0 inset-x-0 z-50 bg-anthracite-950/80 backdrop-blur-lg border-b border-anthracite-700/50">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 md:px-8 h-16">
          ...
          <div className="hidden md:flex gap-6">
            {links.map(([text, href]) => (
              <a key={href} href={href} className="text-sm text-gray-400 hover:text-accent-400 transition-colors">{text}</a>
            ))}
          </div>
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden text-gray-300" aria-label={menuOpen ? 'Закрыть меню' : 'Открыть меню'}>
        ...
        {menuOpen && (
          <div className="md:hidden bg-anthracite-900/95 backdrop-blur-lg border-t border-anthracite-700/50 px-6 pb-6 pt-2 space-y-3">
            {links.map(([text, href]) => (
              <a key={href} href={href} onClick={() => setMenuOpen(false)} className="block text-gray-300 hover:text-accent-400 transition-colors py-1">{text}</a>
            ))}
      ...
      <header className="relative min-h-screen flex items-center justify-center pt-16 overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          <p className="text-accent-400 font-semibold tracking-widest uppercase text-sm mb-4 animate-fade-in-up motion-reduce:animate-none">
            Черновые работы под ключ
          </p>
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-extrabold text-white leading-tight mb-6 animate-fade-in-up [animation-delay:.15s] motion-reduce:animate-none">
            Надёжная основа<br />для <span className="text-accent-500">вашего ремонта</span>
          </h1>
          <p className="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto mb-10 animate-fade-in-up [animation-delay:.3s] motion-reduce:animate-none">
            Профессиональные черновые работы в квартирах и новостройках Москвы. Фиксированные цены, соблюдение сроков, гарантия 2 года.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in-up [animation-delay:.45s] motion-reduce:animate-none">
            ...
            <a href="tel:+74951234567" className="inline-flex items-center justify-center gap-2 border border-anthracite-600 hover:border-accent-500/50 text-gray-300 hover:text-white font-semibold px-8 py-4 rounded-2xl text-lg transition-all hover:bg-anthracite-800">
              <Phone size={18} /> Позвонить сейчас
            </a>
          </div>
          <div className="mt-10 grid gap-3 sm:grid-cols-3 text-left max-w-3xl mx-auto animate-fade-in-up [animation-delay:.6s] motion-reduce:animate-none">
            {highlights.map((item) => (
              <div key={item} className="rounded-2xl border border-anthracite-700/50 bg-anthracite-900/70 px-4 py-3 text-sm text-gray-300">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((service) => {
              ...
              return (
                <article key={service.title} className="group relative bg-anthracite-900 border border-anthracite-700/50 rounded-2xl p-6 transition-all duration-300 hover:border-accent-500/50 hover:-translate-y-1 hover:shadow-lg hover:shadow-accent-500/5 overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="w-12 h-12 rounded-xl bg-accent-500/10 flex items-center justify-center mb-4 group-hover:bg-accent-500/20 transition-colors">
                      <ServiceIcon size={24} className="text-accent-500" />
                    </div>
                    <h3 className="text-lg font-bold text-white mb-2">{service.title}</h3>
                    <p className="text-gray-400 text-sm leading-relaxed mb-4 min-h-[72px]">{service.desc}</p>
                    <div className="flex items-center justify-between gap-3 pt-3 border-t border-anthracite-700/40">
                      <p className="text-accent-400 font-semibold text-sm">от {service.price} ₽/м²</p>
                      <a href="#contact" className="text-xs font-semibold text-gray-300 hover:text-accent-400 transition-colors">
                        Узнать сроки
                      </a>
                    </div>
                  </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-anthracite-900 border border-anthracite-700/50 rounded-2xl p-6 md:p-10">
            <div className="flex flex-col sm:flex-row sm:items-end gap-4 mb-2">
              <label htmlFor="area-range" className="block text-sm font-semibold text-gray-300 flex-1">
              ...
              <label className="text-sm text-gray-400">
                ...
                <input
                  ...
                  step="1"
                  value={area}
                  onChange={(e) => setArea(clampArea(e.target.value))}
                  className="mt-2 w-full sm:w-28 bg-anthracite-800 border border-anthracite-700/50 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-accent-500 transition-colors"
                />
              </label>
            </div>
            <input
            ...
            <div className="grid sm:grid-cols-2 gap-3 mb-8">
              {services.map((service) => {
                ...
                return (
                  <label
                    key={service.title}
                    className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all duration-200 ${selected[service.title] ? 'border-accent-500 bg-accent-500/10' : 'border-anthracite-700/50 hover:border-anthracite-600'}`}
                  >
                    <input type="checkbox" checked={!!selected[service.title]} onChange={() => toggle(service.title)} className="sr-only" />
                    <div className={`w-5 h-5 rounded flex items-center justify-center border transition-colors ${selected[service.title] ? 'bg-accent-500 border-accent-500' : 'border-anthracite-600'}`}>
                      {selected[service.title] && <CheckCircle2 size={14} className="text-white" />}
                    </div>
                    <ServiceIcon size={18} className="text-accent-500" />
                    <span className="text-sm text-gray-200 font-medium flex-1">{service.title}</span>
                    <span className="text-xs text-gray-500">{service.price} ₽/м²</span>
                  </label>
                );
              })}
            </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-8 md:gap-0">
            ...
            {steps.map((step, i) => {
              ...
              return (
                <div key={step.title} className="relative z-10 flex flex-row md:flex-col items-center gap-3 md:gap-4 flex-1">
                  <div className="w-14 h-14 rounded-full bg-anthracite-800 border-2 border-accent-500 flex items-center justify-center shrink-0 transition-transform hover:scale-110">
                    <StepIcon size={22} className="text-accent-400" />
                  </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="relative">
            ...
            <div className="flex items-center justify-center gap-4 mt-6">
              <button onClick={prev} className="w-10 h-10 rounded-full border border-anthracite-700 flex items-center justify-center hover:border-accent-500 hover:text-accent-400 transition-colors" aria-label="Предыдущий отзыв">
                <ChevronLeft size={18} />
              </button>
              ...
              <button onClick={next} className="w-10 h-10 rounded-full border border-anthracite-700 flex items-center justify-center hover:border-accent-500 hover:text-accent-400 transition-colors" aria-label="Следующий отзыв">
      ...
      <Section id="contact" className="py-20 md:py-28 px-4 bg-gradient-to-b from-anthracite-900/50 to-anthracite-950">
        <div className="max-w-xl mx-auto text-center">
          ...
          {submitted ? (
            <div className="bg-green-900/30 border border-green-500/30 rounded-2xl p-8 animate-fade-in-up motion-reduce:animate-none">
            ...
            <form onSubmit={handleSubmit} className="bg-anthracite-900 border border-anthracite-700/50 rounded-2xl p-6 md:p-8 space-y-4">
              <div className="text-left">
                <label htmlFor="lead-name" className="block text-sm font-medium text-gray-300 mb-2">Ваше имя</label>
                <input
                  id="lead-name"
                  type="text"
                  required
              ...
              <div className="text-left">
                <label htmlFor="lead-phone" className="block text-sm font-medium text-gray-300 mb-2">Телефон</label>
                <input
                  id="lead-phone"
                  type="tel"
                  required
              ...
              <div className="text-left">
                <label htmlFor="lead-message" className="block text-sm font-medium text-gray-300 mb-2">Сообщение</label>
                <textarea
                  id="lead-message"
              ...
              {formError && <p className="text-sm text-red-400 text-left">{formError}</p>}
              <button type="submit" disabled={!canSubmit} className="w-full bg-accent-500 hover:bg-accent-600 disabled:bg-anthracite-700 disabled:text-gray-400 disabled:cursor-not-allowed text-white font-bold py-4 rounded-xl text-lg transition-all hover:scale-[1.02] hover:shadow-lg hover:shadow-accent-500/25 active:scale-95 disabled:hover:scale-100 disabled:hover:shadow-none">
                Оставить заявку
              </button>
              <p className="text-xs text-gray-500 text-left">
                Нажимая кнопку, вы соглашаетесь на обратный звонок по указанному номеру.
              </p>
            </form>
          )}
        </div>
      ...
      <footer className="border-t border-anthracite-700/50 py-12 px-4">
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-10">
          ...
          <div className="space-y-3">
            <p className="text-sm font-semibold text-white mb-2">Быстрая связь</p>
            <div className="flex flex-wrap gap-3">
              <a href="tel:+74951234567" className="px-4 py-2 rounded-xl bg-anthracite-800 border border-anthracite-700/50 text-gray-300 hover:text-accent-400 hover:border-accent-500/50 transition-all text-xs font-bold">
                Позвонить
              </a>
              <a href="mailto:info@remontpro.ru" className="px-4 py-2 rounded-xl bg-anthracite-800 border border-anthracite-700/50 text-gray-300 hover:text-accent-400 hover:border-accent-500/50 transition-all text-xs font-bold">
                Написать
              </a>
              <a href="#calc" className="px-4 py-2 rounded-xl bg-anthracite-800 border border-anthracite-700/50 text-gray-300 hover:text-accent-400 hover:border-accent-500/50 transition-all text-xs font-bold">
                Калькулятор
              </a>
            </div>
          </div>
